...
Page({
  ...
  login: function () {
    ...
    app.api.request({
      ...
      callBack: _ => {
        app.storage.setKey('name', name);
        app.storage.setKey('password', password);
        if (!(app.storage.getKey('accounts') instanceof Array)) {
          app.storage.setKey('accounts', []);
        }
        app.storage.getKey('accounts').push({
          name,
          password,
        });
        app.storage.save();
  ...
  onLoad: function () {
    app.eventBus.on('showPassword', this, () => {
      this.setData({
        showPassword: true,
      });
    });
