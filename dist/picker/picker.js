...
Component({
  ...
  methods: {
    ...
    _getIndexByIdOfObject(listData, idArr, key, arr) {
      ...
      for (let i = 0, len = listData.length; i < len; i++) {
        if (listData[i][key] === idArr[arr.length][key]) {
          arr.push(i);
          return this._getIndexByIdOfObject(
            listData[i].children,
            idArr,
            key,
            arr
      ...
      switch (scrollType) {
        ...
        case 'link':
          ...
          if (
            ...
            defaultPickData.every((v, i) => isPlainObject(v))
          ) {
            const key = (this.data.onlyKey = Object.keys(
              defaultPickData[0]
            )[0]);
    ...
    _computedBackData(backData) {
      ...
      if (scrollType === 'normal') {
        return backData.map((v, i) =>
          listData[i].findIndex((vv, ii) => this._compareObj(v, vv))
        );
      } else {
    ...
    _compareDate() {
      ...
      return (
        defaultPickDataTemp === defaultPickData && listDataTemp === listData
      );
    },
    _setTempData() {
      const { defaultPickData, listData } = this.properties;
